...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case FORLOOP:
      if (record->event.pressed) {
        SEND_STRING("for (i = 0; i < array.length; i++) {}");
        tap_code(KC_LEFT);
        tap_code(KC_ENT);
      }
      break;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_FN3] = LAYOUT( /* F keys */
    ...
    KC_TRANSPARENT, KC_TRANSPARENT, SCHEDULESTATUS, KC_TRANSPARENT, FORLOOP,        KC_TRANSPARENT, KC_F4,          KC_F5,          KC_F6,          KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT,
